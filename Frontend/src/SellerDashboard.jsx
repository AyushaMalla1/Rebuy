...
function SellerDashboard() {
  ...
  const fetchVerifications = async (sellerId) => {
    ...
    try {
      const response = await fetch(`${API_URL}/sellers/${sellerId}/verifications`);
      const data = await response.json();
  ...
  const markAllAsRead = async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/notifications/${user._id}/read-all`, {
        method: 'PATCH'
      });
        ...
        {activeTab === 'verifications' && (
          <div className="verifications-section">
            <div className="verification-info-banner">
              <FaInfoCircle style={{ fontSize: '24px', color: '#00bcd4' }} />
              <div>
                <h3>About Product Verifications</h3>
                <p>Customers verify product conditions after delivery. These verifications are reviewed by admins before becoming public. You can view all verifications for your products here.</p>
              </div>
            </div>
            ...
            <div className="verifications-stats">
              <div className="stat-card">
                ...
                <p className="stat-number">{verificationStats.total}</p>
              </div>
              <div className="stat-card pending">
                <h4>Pending Review</h4>
                <p className="stat-number">{verificationStats.pending}</p>
              </div>
              <div className="stat-card approved">
                <h4>Approved</h4>
                <p className="stat-number">{verificationStats.approved}</p>
              </div>
              <div className="stat-card rejected">
                <h4>Rejected</h4>
                <p className="stat-number">{verificationStats.rejected}</p>
              </div>
            </div>
            ...
            <div className="verifications-list">
              {loadingVerifications ? (
                ...
                <div className="empty-state">
                  <FaCheckCircle style={{ fontSize: '64px', color: '#00bcd4', marginBottom: '20px' }} />
                  <h3>No Verifications Yet</h3>
                  <p>Customer product verifications will appear here once they verify their orders.</p>
                </div>
              ) : (
                  ...
                  <tbody>
                    {verifications.map((verification) => (
                      <tr key={verification._id}>
                        <td>#{verification.orderId || 'N/A'}</td>
                        <td>{verification.customerName || 'Unknown'}</td>
                        ...
                        <td>
                          <span className={`status-badge ${verification.matchesDescription === 'yes' ? 'success' : 'danger'}`}>
                            {verification.matchesDescription === 'yes' ? 'Good' : 'Bad'}
                          </span>
                        </td>
                        <td>
                          <span className={`status-badge ${
                            ...
                            {verification.approvalStatus === 'approved' && 'Approved'}
                            {verification.approvalStatus === 'rejected' && 'Rejected'}
                            {verification.approvalStatus === 'pending' && 'Pending'}
                          </span>
                        </td>
                ...
                {selectedMessage ? (
                  <>
                    ...
                    <div className="chat-input-container">
                      ...
                      <div className="message-input-wrapper-seller">
                        {attachedFile && (
