...
import {
  ...
  FiSettings, FiLogOut, FiShield, FiEdit2, FiTrash2,
  FiEye, FiCheckCircle, FiXCircle, FiSearch, FiFilter, FiDownload,
  FiBarChart2, FiActivity, FiClock, FiAlertCircle,
...
function AdminDashboard() {
  ...
  const handleCompletePayout = async (payoutId) => {
    const transactionRef = prompt('Enter transaction reference/ID:');
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5000/api/payouts/${payoutId}/complete`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleCancelPayout = async (payoutId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5000/api/payouts/${payoutId}/cancel`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const triggerWeeklyPayout = async () => {
    if (!window.confirm('Trigger weekly payout generation for all sellers?')) return;

    try {
      const token = localStorage.getItem('token');
      const response = await fetch('http://localhost:5000/api/payouts/admin/trigger-weekly', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        }
      ...
      alert('Failed to trigger payout generation');
    }
  };
  ...
  const fetchVerifications = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('http://localhost:5000/api/admin/verifications/all', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const handleRejectVerification = async (verificationId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5000/api/admin/verifications/${verificationId}/reject`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      <main className="admin-main">
          ...
          <div className="content-section">
            {verifications.length === 0 ? (
              <div className="empty-state">
                ...
                <p>Customer condition verifications will appear here for review</p>
              </div>
            ) : (
              <div className="table-container">
                <table className="data-table">
                  ...
                  <tbody>
                    {verifications.map((verification) => (
                      <tr key={verification._id}>
                        <td>#{verification.orderId?.slice(-6) || 'N/A'}</td>
                        <td>
                          <div className="user-cell">
                            <FiUser />
                            <span>{verification.customerName || 'Unknown'}</span>
                          </div>
                        </td>
                        <td>{verification.productName || 'Unknown Product'}</td>
                        <td>
                          <span style={{color: '#ffc107', fontSize: '16px'}}>
                            {'★'.repeat(verification.rating || 0)}{'☆'.repeat(5 - (verification.rating || 0))}
                          </span>
                        </td>
                        <td>
                          <span className={`status-badge ${verification.matchesDescription === 'yes' ? 'success' : 'danger'}`}>
                            {verification.matchesDescription === 'yes' ? (
                              <><FiCheckCircle /> Good</>
                            ) : (
                              <><FiXCircle /> Bad</>
                            )}
                        ...
                        <td>
                          <span className={`status-badge ${
                            verification.approvalStatus === 'approved' ? 'success' : 
                            verification.approvalStatus === 'rejected' ? 'danger' : 
                            'warning'
                          }`}>
                            {verification.approvalStatus === 'approved' && <><FiCheckCircle /> Approved</>}
                            {verification.approvalStatus === 'rejected' && <><FiXCircle /> Rejected</>}
                            {verification.approvalStatus === 'pending' && <><FiClock /> Pending</>}
                          </span>
                        </td>
                        <td>
                          <div className="action-buttons">
                            ...
                            {verification.approvalStatus === 'pending' && (
                              <>
                                <button 
          ...
          <div className="content-section">
            ...
            <div className="stats-grid" style={{marginBottom: '30px'}}>
              <div className="stat-card">
                ...
                <div className="stat-details">
                  ...
                  <h3 className="stat-value">Rs. {payoutStats.pendingAmount.toLocaleString()}</h3>
                </div>
              </div>
              <div className="stat-card">
                ...
                <div className="stat-details">
                  ...
                  <h3 className="stat-value">Rs. {payoutStats.completedAmount.toLocaleString()}</h3>
                </div>
              </div>
            </div>
            ...
            <div style={{marginBottom: '20px', display: 'flex', gap: '10px'}}>
              <button 
                ...
                onClick={triggerWeeklyPayout}
              >
                <FiDollarSign /> Generate Weekly Payouts
              </button>
              <button 
            ...
            <div className="table-container">
              <table className="data-table">
                ...
                <tbody>
                  {payouts.map(payout => (
                    <tr key={payout._id}>
                      <td className="user-name">{payout.sellerName}</td>
                      <td>{payout.seller?.storeName || 'N/A'}</td>
                      ...
                      <td>
                        <span className={`status-badge ${payout.status}`}>
                          ...
                          {payout.status === 'completed' && <FiCheckCircle />}
                          {payout.status === 'cancelled' && <FiXCircle />}
                          {payout.status}
                        </span>
                      </td>
                      <td>
            ...
            {payouts.length === 0 && (
              <div className="empty-state-card">
                ...
                <p>Payout requests will appear here</p>
              </div>
            )}
          </div>
        )}
            ...
            <div className="modal-content" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="modal-body">
                <div className="detail-grid">
                  <div className="detail-item">
                    <label>Seller:</label>
                    <span>{selectedPayout.sellerName}</span>
                  </div>
                  <div className="detail-item">
                    <label>Store:</label>
                    <span>{selectedPayout.seller?.storeName || 'N/A'}</span>
                  </div>
                  <div className="detail-item">
                    <label>Amount:</label>
                    <span><strong>Rs. {selectedPayout.amount.toLocaleString()}</strong></span>
                  </div>
                  <div className="detail-item">
                    <label>Commission:</label>
                    <span>Rs. {selectedPayout.platformCommission?.toLocaleString() || 0}</span>
                  </div>
                  <div className="detail-item">
                    <label>Method:</label>
                    <span>{selectedPayout.payoutMethod.toUpperCase()}</span>
                  </div>
                  <div className="detail-item">
                    <label>Status:</label>
                  ...
                  <div className="detail-item">
                    <label>Orders:</label>
                    <span>{selectedPayout.orders?.length || 0} orders</span>
                  </div>
                  <div className="detail-item">
                    <label>Requested:</label>
                    <span>{new Date(selectedPayout.requestedAt || selectedPayout.createdAt).toLocaleString()}</span>
                  </div>
                  {selectedPayout.completedAt && (
                    <div className="detail-item">
                      <label>Completed:</label>
                      <span>{new Date(selectedPayout.completedAt).toLocaleString()}</span>
                    </div>
                  )}
                  {selectedPayout.khaltiTransactionId && (
                    <div className="detail-item">
                      <label>Transaction ID:</label>
                      <span>{selectedPayout.khaltiTransactionId}</span>
                    </div>
                  )}
                </div>
                ...
                <div style={{marginTop: '20px'}}>
                  ...
                  {selectedPayout.payoutMethod === 'bank' && selectedPayout.bankDetails && (
                    <div className="detail-grid">
                      <div className="detail-item">
                        <label>Bank:</label>
                        <span>{selectedPayout.bankDetails.bankName}</span>
                      </div>
                      <div className="detail-item">
                      ...
                      <div className="detail-item">
                        ...
                        <span>{selectedPayout.bankDetails.accountName}</span>
                      </div>
                      <div className="detail-item">
                        <label>Branch:</label>
                        <span>{selectedPayout.bankDetails.branchName}</span>
                      </div>
                    </div>
                  )}
                  {selectedPayout.payoutMethod === 'khalti' && (
                    <div className="detail-item">
                      <label>Khalti Mobile:</label>
                      <span>{selectedPayout.khaltiMobile}</span>
                    </div>
                  )}
                  {selectedPayout.payoutMethod === 'esewa' && selectedPayout.seller?.payoutDetails?.esewaId && (
                    <div className="detail-item">
                      <label>eSewa ID:</label>
                      <span>{selectedPayout.seller.payoutDetails.esewaId}</span>
                    </div>
                  )}
                </div>
        ...
        {showVerificationModal && selectedVerification && (
          <div className="modal-overlay" onClick={() => setShowVerificationModal(false)}>
            <div className="modal-content" onClick={(e) => e.stopPropagation()}>
              <div className="modal-header">
                <h2>Condition Verification Details</h2>
              ...
              <div className="modal-body">
                <div className="detail-grid">
                  <div className="detail-item">
                    ...
                    <span>#{selectedVerification.orderId?.slice(-8) || 'N/A'}</span>
                  </div>
                  <div className="detail-item">
                    <label>Customer:</label>
                    <span>{selectedVerification.customerName || 'Unknown'}</span>
                  </div>
                  <div className="detail-item">
                    <label>Product:</label>
                    <span>{selectedVerification.productName || 'Unknown Product'}</span>
                  </div>
                  <div className="detail-item">
                    <label>Seller:</label>
                    <span>{selectedVerification.sellerName || 'Unknown'}</span>
                  </div>
                  <div className="detail-item">
                    <label>Condition Status:</label>
                    <span className={`status-badge ${selectedVerification.matchesDescription === 'yes' ? 'success' : 'danger'}`}>
                      {selectedVerification.matchesDescription === 'yes' ? (
                        <><FiCheckCircle /> Good - As Described</>
                      ) : (
                        <><FiXCircle /> Bad - Not As Described</>
                      )}
                    </span>
                  </div>
                  <div className="detail-item">
                    ...
                    <span style={{fontSize: '20px', color: '#ffc107'}}>
                      {'★'.repeat(selectedVerification.rating || 0)}
                      {'☆'.repeat(5 - (selectedVerification.rating || 0))}
                      <span style={{marginLeft: '8px', fontSize: '14px', color: '#666'}}>
                        ({selectedVerification.rating || 0}/5)
                      </span>
                    </span>
                  </div>
                  <div className="detail-item">
                    ...
                    <span className={`status-badge ${
                      selectedVerification.approvalStatus === 'approved' ? 'success' : 
                      selectedVerification.approvalStatus === 'rejected' ? 'danger' : 
                      'warning'
                    }`}>
                      {selectedVerification.approvalStatus === 'approved' && <><FiCheckCircle /> Approved</>}
                      {selectedVerification.approvalStatus === 'rejected' && <><FiXCircle /> Rejected</>}
                      {selectedVerification.approvalStatus === 'pending' && <><FiClock /> Pending Review</>}
                    </span>
                  </div>
                  <div className="detail-item">
                    <label>Public:</label>
                    <span>{selectedVerification.isPublic ? 'Yes' : 'No (Awaiting Approval)'}</span>
                  </div>
                  <div className="detail-item">
                  ...
                  {selectedVerification.approvedAt && (
                    <div className="detail-item">
                      ...
                      <span>{new Date(selectedVerification.approvedAt).toLocaleString()}</span>
                    </div>
                  )}
                  {selectedVerification.approvedBy && (
                    <div className="detail-item">
                ...
                <div style={{marginTop: '20px'}}>
                  <h3 style={{marginBottom: '10px', fontSize: '16px', fontWeight: '600'}}>Customer Notes</h3>
                  <p style={{padding: '12px', background: '#f5f5f5', borderRadius: '8px', border: '1px solid #e0e0e0', lineHeight: '1.6', color: '#333'}}>
                    {selectedVerification.customerNotes || 'No notes provided'}
                  </p>
                </div>

                {/* Verification Images - ALWAYS SHOW */}
                <div style={{marginTop: '20px'}}>
                  <h3 style={{marginBottom: '10px', fontSize: '16px', fontWeight: '600'}}>
                    Product Images ({selectedVerification.verificationImages?.length || 0})
                  </h3>
                  {selectedVerification.verificationImages && selectedVerification.verificationImages.length > 0 ? (
                    <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '12px', marginTop: '10px'}}>
                      {selectedVerification.verificationImages.map((img, idx) => (
                        <div key={idx} style={{position: 'relative', paddingTop: '100%', borderRadius: '8px', overflow: 'hidden', border: '2px solid #e0e0e0', cursor: 'pointer'}} onClick={() => window.open(img, '_blank')}>
                          <img 
                            src={img} 
                    ...
                    <p style={{padding: '12px', background: '#fff3cd', borderRadius: '8px', color: '#856404'}}>
                      No images uploaded
                    </p>
                  )}
                </div>
                ...
                {selectedVerification.rejectionReason && (
                  <div style={{marginTop: '20px'}}>
                    ...
                    <p style={{padding: '10px', background: '#fff3cd', borderRadius: '5px', color: '#856404'}}>
                      {selectedVerification.rejectionReason}
                    </p>
                  </div>
                )}
                ...
                {/* Action Buttons */}
                {selectedVerification.approvalStatus === 'pending' && (
                  <div style={{marginTop: '20px', display: 'flex', gap: '10px', justifyContent: 'flex-end'}}>
                    <button 
                      className="action-btn reject"
                      onClick={() => {
                        setShowVerificationModal(false);
                        handleRejectVerification(selectedVerification._id);
                      }}
                    >
                      <FiXCircle /> Reject
                    </button>
                    ...
                    >
                      <FiCheckCircle /> Approve & Make Public
                    </button>
                  </div>
                )}
              </div>
